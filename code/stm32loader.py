...
class CommandInterface:
    def open(self, aport='/dev/ttyUSB0', abaudrate=115200) :
        self.sp = serial.Serial(
            ...
            timeout=30               # set a timeout value, None for waiting forever
        )
    ...
    def cmdExtendedEraseMemory(self, sectors=None):
        if self.cmdGeneric(0x44):
            ...
            if sectors is None:
                ...
                print "per-sector erasing not yet supported"
                return
            self._wait_for_ask("0x44 extended erasing failed")
            mdebug(10, "    Extended erase done")
        else:
